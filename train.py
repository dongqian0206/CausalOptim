...
def main():
    ...
    fig, ax = plt.subplots()
    ax.plot(ll_x2y, color='C0', label=r'$X \rightarrow Y$', linewidth=2)
    ax.plot(ll_y2x, color='C3', label=r'$Y \rightarrow X$', linewidth=2)
    ...
    plt.savefig('./plot/mle.pdf', bbox_inches='tight', format='pdf')
    plt.show()
    ...
    loss_x2y = np.asarray([result.loss_x2y for result in results])
    loss_y2x = np.asarray([result.loss_y2x for result in results])

    fig, ax = plt.subplots()
    ax.plot(sig_alphas, linewidth=2, color='k', label='N = {0}'.format(args.finetune_n_iters))
    ...
    plt.savefig('./plot/transfer.pdf', bbox_inches='tight', format='pdf')
    plt.show()

    fig, ax = plt.subplots()
    ax.plot(loss_x2y, color='C0', label=r'$X \rightarrow Y$', linewidth=2)
    ...
    plt.savefig('./plot/loss_x2y.pdf', bbox_inches='tight', format='pdf')
    plt.show()

    fig, ax = plt.subplots()
    ax.plot(loss_y2x, color='C3', label=r'$Y \rightarrow X$', linewidth=2)
    ...
    plt.savefig('./plot/loss_y2x.pdf', bbox_inches='tight', format='pdf')
    plt.show()
